...
#new_data = new_data[857105:] #2011~20150325
new_data.index = range(len(new_data)) #index:0~最後
#求出RSV
...
def data_helper(df, pastday, futureday):
    ...
    for i in range(df.shape[0] - pastday - futureday):
        ...
        X_train.append(np.array(df.iloc[i : i + pastday,2:7]))
        #從樣本中抽取出NextClose
...
def build_model(shape):
    ...
    model.add(keras.layers.convolutional.Conv1D(filters=32, kernel_size=(1), strides=(1), padding='valid', input_shape=(1,5), activation="relu"))
...
X_train, Y_train = data_helper(data_norm, 1, 1)
...
train_history = model.fit(X_train, Y_train, batch_size = 128, 
                           epochs=1000, validation_split = 0.2, callbacks = [callback], verbose=1)
#儲存訓練好的模型
model.save('CNN.h5')
#讀取模型
model_lstm = load_model('CNN.h5')
    #買進訊號
...
def backtest(dataframe, data_no, model_ls):
    ...
    df_backtest = data_no.iloc[:int(len(data_norm.index) * 0.8),2:-1]
    #df_backtest.index = range(len(df_backtest))
    X_backtest = np.array(df_backtest.iloc[:,0:5])
            ...
            if i >= 2:
                ...
                #符合死亡交叉後接著判斷今天的收盤價是否有小於昨日的最低價，有的話就賣出
                    ...
                    if (dataframe['Close'][i + 1] - reg) > 0:
                        ...
                        right_call += 1
                        if ((dataframe['Close'][i + 1] - reg) * 200) > max_right_score:                            

                                max_right_score=((dataframe['Close'][i + 1] - reg) * 200)
                                bx=bx+[a]
                                by=by+['%.2f' %Y_test[a]]                            
                    else:
                        wrong_score = wrong_score + ((reg - dataframe['Close'][i + 1]) * 200)
                        wrong_call += 1
                        if ((reg - dataframe['Close'][i + 1]) * 200) > max_wrong_score:                           
                                max_wrong_score=((dataframe['Close'][i + 1] - reg) * 200)
                                sx=sx+[a]
                                sy=sy+['%.2f'%Y_test[a]]                            
...
#    plt.axis([0, 240000, 0, 1])
#    df_backtest['deposit'].plot(ax = ax1, linewidth = 3, color = 'k')     

    list1 = list((map(float,by)))
    list2 = list((map(float,sy)))
    plt.plot(Y_test,color='black')
# Plot the buy and sell signals on the same plot
    plt.plot(bx,list1, 'v', markersize=10, color='r')
    plt.plot(sx,list2, '^', markersize=10, color='b')
    plt.legend(loc=0)
# Display everything
    plt.show()
    ...
    print("最大收益:",max_right_score) 
    print("最大損失:",max_wrong_score)
 #   print("平均收益:",(right_score/right_call))
    print("平均損失:",(wrong_score/wrong_call))
    print(bx)
    print(by)
    print(sx)
    print(sy)
...
plt.plot(Y_pred,color='red', label='CNN Answer')
